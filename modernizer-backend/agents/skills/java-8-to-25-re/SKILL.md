...
8. **Packaging & Deployment Model** — `<packaging>` value in `pom.xml` (or the Gradle equivalent); whether `src/main/webapp/WEB-INF/web.xml` exists and, if so, list every `<servlet>`/`<filter>`/`<listener>`/`<security-constraint>` it declares; whether the app already has a `SpringBootServletInitializer`. This tells the planner whether a Spring Boot upgrade (if requested) should target a standalone embedded-server JAR or a WAR deployed to an external servlet container — do not skip this even if Spring Boot upgrade wasn't requested, since it's cheap to record and the plan may still need it.

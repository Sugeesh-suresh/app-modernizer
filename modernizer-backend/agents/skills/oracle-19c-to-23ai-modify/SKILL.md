...
4. When modifying Java or other application code, apply the following general code quality and security best practices:
...
5. Do not change business logic (data validation rules, calculation logic, trigger semantics) beyond what the plan calls for.
6. Never write an `ALTER SYSTEM`, `ALTER DATABASE`, or Data Pump script — those are explicitly out of scope per the plan.
7. If a file listed in the manifest turns out not to need any change after reading it, skip writing it and note that in your summary.
